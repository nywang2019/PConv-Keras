...
class PConvUnet(object):
    ...
    def build_pconv_unet(self, train_bn=True):      
        ...
        def encoder_layer(img_in, mask_in, filters, kernel_size, bn=True):
            ...
            return conv, mask
        def encoder_layer_1by1(img_in, mask_in, filters, bn=True):
            conv, mask = PConv2D(filters, kernel_size=1, strides=1, padding='same')([img_in, mask_in])
            if bn:
                conv = BatchNormalization(name='EncBN'+str(encoder_layer.counter))(conv, training=train_bn)
            conv = Activation('relu')(conv)
            encoder_layer.counter += 1
        ...
        e_conv1a, e_mask1a = encoder_layer_1by1(inputs_img, inputs_mask, 64, bn=False)
        e_conv1a, e_mask1a = encoder_layer(e_conv1a, e_mask1a, 64, 5)
        e_conv1b, e_mask1b = encoder_layer_1by1(inputs_img, inputs_mask, 64, bn=False)
        e_conv1b, e_mask1b = encoder_layer(e_conv1b, e_mask1b, 64, 3)
        e_conv1c, e_mask1c = encoder_layer(inputs_img, inputs_mask, 64, 1, bn=False)
        #e_conv1 = Concatenate(axis=3)([e_conv1a, e_conv1b, e_conv1c])
        #e_mask1 = Concatenate(axis=3)([e_mask1a, e_mask1b, e_mask1c])
        e_conv1d = MaxPool2D(2,None,padding='same')(inputs_img)
        #e_mask1d = MaxPool2D(2,None,padding='same')(inputs_mask)
        e_mask1d = AvgPool2D(2, None, padding='same')(inputs_mask)
        e_conv1d, e_mask1d = encoder_layer_1by1(e_conv1d, e_mask1d, 64)
        e_conv1 = Concatenate(axis=3)([e_conv1a, e_conv1b, e_conv1c,e_conv1d])
        e_mask1 = Concatenate(axis=3)([e_mask1a, e_mask1b, e_mask1c,e_mask1d])

        def inception(inputs_img, inputs_mask, filters):
            e_conva, e_maska = encoder_layer_1by1(inputs_img, inputs_mask, 64)
            e_conva, e_maska = encoder_layer(e_conva, e_maska, filters, 5)
            e_convb, e_maskb = encoder_layer_1by1(inputs_img, inputs_mask, 64)
            e_convb, e_maskb = encoder_layer(e_convb, e_maskb, filters, 3)
            e_convc, e_maskc = encoder_layer(inputs_img, inputs_mask, filters, 1)
            #e_conv = Concatenate(axis=3)([e_conva, e_convb, e_convc])
            #e_mask = Concatenate(axis=3)([e_maska, e_maskb, e_maskc])
            e_convd = MaxPool2D(2, None, padding='same')(inputs_img)
            #e_maskd = MaxPool2D(2, None, padding='same')(inputs_mask)
            e_maskd = AvgPool2D(2, None, padding='same')(inputs_mask)
            e_convd, e_maskd = encoder_layer_1by1(e_convd, e_maskd, filters)
            e_conv = Concatenate(axis=3)([e_conva, e_convb, e_convc, e_convd])
            e_mask = Concatenate(axis=3)([e_maska, e_maskb, e_maskc, e_maskd])
            return e_conv, e_mask            

        e_conv2, e_mask2 = inception(e_conv1, e_mask1, 128)
        e_conv3, e_mask3 = inception(e_conv2, e_mask2, 256)
        e_conv4, e_mask4 = inception(e_conv3, e_mask3, 512)
        #e_conv5, e_mask5 = inception(e_conv4, e_mask4, 512)
        #e_conv6, e_mask6 = inception(e_conv5, e_mask5, 512)
        #e_conv7, e_mask7 = inception(e_conv6, e_mask6, 512)
        #e_conv8, e_mask8 = inception(e_conv7, e_mask7, 512)
        
        
        # DECODER Inception version [2020 WNY]
        ...
        #d_conv9, d_mask9 = decoder_layer(e_conv8, e_mask8, e_conv7, e_mask7, 512)
        #d_conv10, d_mask10 = decoder_layer(d_conv9, d_mask9, e_conv6, e_mask6, 512)
        #d_conv11, d_mask11 = decoder_layer(d_conv10, d_mask10, e_conv5, e_mask5, 512)
        #d_conv12, d_mask12 = decoder_layer(d_conv11, d_mask11, e_conv4, e_mask4, 512)
        #d_conv13, d_mask13 = decoder_layer(d_conv12, d_mask12, e_conv3, e_mask3, 256)
        #d_conv14, d_mask14 = decoder_layer(d_conv13, d_mask13, e_conv2, e_mask2, 128)
        #d_conv15, d_mask15 = decoder_layer(d_conv14, d_mask14, e_conv1, e_mask1, 64)
        #d_conv16, d_mask16 = decoder_layer(d_conv15, d_mask15, inputs_img, inputs_mask, 3, bn=False)
        #outputs = Conv2D(3, 1, activation = 'sigmoid', name='outputs_img')(d_conv16)
        d_conv1, d_mask1 = decoder_layer(e_conv4, e_mask4, e_conv3, e_mask3, 256)
        d_conv2, d_mask2 = decoder_layer(d_conv1, d_mask1, e_conv2, e_mask2, 128)
        d_conv3, d_mask3 = decoder_layer(d_conv2, d_mask2, e_conv1, e_mask1, 64)
        d_conv4, d_mask4 = decoder_layer(d_conv3, d_mask3, inputs_img, inputs_mask, 3, bn=False)        
        outputs = Conv2D(3, 1, activation = 'sigmoid', name='outputs_img')(d_conv4)
