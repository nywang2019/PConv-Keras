...
class PConvUnet(object):
    ...
    def build_pconv_unet(self, train_bn=True):      
        ...
        def inception(inputs_img, inputs_mask, filters,bn=True):
            e_conva, e_maska = encoder_layer(inputs_img, inputs_mask, filters, 5,bn)
            e_convb, e_maskb = encoder_layer(inputs_img, inputs_mask, filters, 3,bn)
            e_convc, e_maskc = encoder_layer(inputs_img, inputs_mask, filters, 1,bn)
        ...
        e_conv1, e_mask1 = inception(inputs_img, inputs_mask, 64, bn=False)
        ...
        def decoder_layer(img_in, mask_in, e_conv, e_mask, filters, bn=True):
            ...
            conva, maska = PConv2D(filters, kernel_size=5, padding='same')([concat_img, concat_mask])
            convb, maskb = PConv2D(filters, kernel_size=3, padding='same')([concat_img, concat_mask])
    ...
    def loss_total(self, mask):
        ...
        def loss(y_true, y_pred):
            ...
            # return l1 + 6*l2 + 0.05*l3 + 120*(l4+l5) + 0.1*l6 author's original loss
