...
class PConvUnet(object):
    ...
    def build_pconv_unet(self, train_bn=True):      
        ...
        # first inception module(it was not defined as a block):
        ...
        e_conv1 =e_conv1a+e_conv1b+e_conv1c
        e_mask1 =e_mask1a+e_mask1b+e_mask1c
        ...
        def inception(inputs_img, inputs_mask, filters):
            ...
            e_conv =e_conva+ e_convb+ e_convc
            e_mask =e_maska+ e_maskb+ e_maskc
        ...
        def decoder_layer(img_in, mask_in, e_conv, e_mask, filters, bn=True):
            ...
            conv =tf.add(conva,convb,convc)
            mask =tf.add(maska,maskb,maskc)
    ...
    def loss_total(self, mask):
        ...
        def loss(y_true, y_pred):
            ...
            #return l1 + 6*l2 + 0.05*l3 + 120*(l4+l5) + 0.1*l6  (original one)
            # return l1 + 6*l2 + 0.05*l3 + l4+l5 (my pretrain)
            # return l1 + 6*l2 + 0.05*l3 + 120*(l4+l5)  (my finetune)
            # the following loss is used in the p1t19 (same as the original one):
