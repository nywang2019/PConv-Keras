...
class MaskGenerator():
    ...
    def _generate_mask(self):
        ...
        for _ in range(randint(1, 10)):
            ...
            radius = randint(size, size*4)
            cv2.circle(img,(x1,y1),radius,(1,1,1), -1)

        # Draw random rectangles
        #for _ in range(randint(1, 2)):
            #x1, y1 = randint(1, self.width), randint(1, self.height)
            #length = randint(size*3, size*5)
        ...
        for _ in range(randint(1, 10)):
            ...
            a1, a2, a3 = randint(0, 180), randint(size, 180), randint(size, 180)
            thickness = randint(size, size*4)
            cv2.ellipse(img, (x1,y1), (s1,s2), a1, a2, a3,(1,1,1), thickness)
        
        # Draw random solid ellipses ?added by wny?
        for _ in range(randint(1, 10)):
            x1, y1 = randint(1, self.width), randint(1, self.height)
            #s1, s2 = randint(1, self.width), randint(1, self.height)
            s1, s2 = randint(size, size*4), randint(size, size*4)
