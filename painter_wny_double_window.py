...
class Paint(object):
    ...
    def __init__(self, config):
        ...
        self.root = Tk()
        self.root.title("Thanka Inpainting (V1.0)")
        ...
        self.poly_button1 = Button(self.root, text='Pen 1', command=self.use_poly, width=12, height=3)
        self.poly_button1.grid(row=1, column=2)

        self.poly_button2 = Button(self.root, text='Pen 2', command=self.pen_size2, width=12, height=3)
        self.poly_button2.grid(row=2, column=2)

        self.poly_button3 = Button(self.root, text='Pen 3', command=self.pen_size3, width=12, height=3)
        self.poly_button3.grid(row=3, column=2)
    ...
    def setup(self):
        ...
        self.active_button = self.poly_button1
        ...
        # self.model.load(r"D:\PycharmProjects2\PConv-Keras\data\logs\Thanka_phase1\p1t165\weights.46-1.23.h5",   train_bn=False)
        # self.model.load(r"D:\PycharmProjects2\PConv-Keras\data\logs\Thanka_phase1\p1t39\weights.16-1.13.h5", train_bn=False)
        # wny
        ###############################################################
        print('Model loaded.')
    ...
    def save(self):
        ...
        self.masked_img.save(self.filepath+'/' + self.filename_ + '_masked_input.png')
        ...
        cv2.imwrite(os.path.join(self.filepath, self.filename_ + '_inpainted_result.png'), cv2.cvtColor(self.predicted_img, cv2.COLOR_BGR2RGB))
    ...
    def fill(self):
        ...
        self.masked_img=image_temp
        # self.masked_img.save('./imgs/generated_input_image.png')


        #cv2.imwrite('./imgs/generated_mask_image.png', (1-self.mask)*255)
        #cv2.imwrite(self.filepath+ self.filename_ + '_mask.png', (1 - self.mask) * 255)
        #########################################################################
        # wny
        ...
        cv2.imwrite(self.filepath+'/'+'temp_result.png', cv2.cvtColor(self.predicted_img, cv2.COLOR_BGR2RGB))

        ##########################################################################
        #photo = Image.open('./imgs/result.png')
        photo = Image.open(self.filepath+'/'+'temp_result.png')
    ...
    def use_poly(self):
        self.activate_button(self.poly_button1)
        self.line_width = 24
        self.mode = 'poly'
